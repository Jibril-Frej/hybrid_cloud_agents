...
roadmap, and [`specs/v4-spec.md`](specs/v4-spec.md) for the current version.

## Status

**V4** — public retrieval on top of V3's private retrieval. The public
worker now queries its own local Chroma vector store, seeded from synthetic
documents in `data/public/`, and returns the matching chunks instead of a
canned response. The orchestrator combines those public chunks with its own
locally-retrieved private chunks, same templated combination as V3. The
one-way membrane boundary test continues to apply unchanged: only the raw
query crosses to the public worker, regardless of what either side retrieves.
The full prior prototype (LangGraph + Chroma + local LLM) is preserved on the
`archive/v1-original` branch and will be reintroduced incrementally in later
versions.

## Architecture (V1-V4)

Two independent `kind` clusters, one mutually-authenticated TLS call between
them. The orchestrator forwards only the raw query to the public worker, and
separately retrieves private context from a local Chroma index; the public
worker retrieves its own context from a separate local Chroma index.
...
flowchart LR
    ...
    subgraph Private["Private cluster (kind-private)"]
        ...
        PrivateChroma["private Chroma index\n(data/chroma/private)"]
        Orchestrator -->|"query"| PrivateChroma
        PrivateChroma -->|"private chunks"| Orchestrator
    end

    subgraph Public["Public cluster (kind-public)"]
        PublicWorker["public-worker\nFastAPI :8001"]
        PublicChroma["public Chroma index\n(data/chroma/public)"]
        PublicWorker -->|"query"| PublicChroma
        PublicChroma -->|"public chunks"| PublicWorker
    end
...
The private and public Chroma indexes are built from `data/private/` and
`data/public/` (git-tracked synthetic documents) via `make seed`, or lazily
on first query if they don't exist yet. Each is persisted under
`data/chroma/` (gitignored) and stays in its own cluster — private chunks
never leave the private cluster, and public chunks flow inward to the
orchestrator freely (see [`specs/v3-spec.md`](specs/v3-spec.md) and
[`specs/v4-spec.md`](specs/v4-spec.md)).
...
- **chromadb (ONNX `all-MiniLM-L6-v2`)** — local vector store, one index per
  side: V3 added the private index, V4 mirrors it with a public index.
  Chroma's default embedding function bundles its own ONNX model, so no
  separate model download or GPU is needed; each index is a persistent
  on-disk directory (`data/chroma/private`, `data/chroma/public`, both
  gitignored), seeded from `data/private/`/`data/public/` by `make seed` or
...
(private cluster) and asserts it gets back the public worker's retrieved
public chunks combined with locally-retrieved private context, over mTLS,
proving the cross-cluster path and both retrieval paths work end to end. The
...
- **`tests/integration/e2e/`** (`test_kind_clusters.py`) — marked
  ...
  combines the public worker's retrieved chunks with the orchestrator's
  private context — proving the full topology (build → load → deploy →
  cross-cluster HTTP → response) works end to end, not just the application
  code in isolation.
