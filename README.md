...
roadmap, and [`specs/v3-spec.md`](specs/v3-spec.md) for the current version.

## Status

**V3** — private retrieval on top of the V1/V2 two-`kind`-cluster plumbing
with mTLS. The orchestrator now queries a local Chroma vector store seeded
from synthetic documents in `data/private/`, and combines the retrieved
chunks with the public worker's canned response. This is the first version
where the one-way membrane boundary test is meaningful: private content
exists and is proven to never cross outward. The full prior prototype
(LangGraph + Chroma + local LLM) is preserved on the `archive/v1-original`
branch and will be reintroduced incrementally in later versions.
...
separately retrieves private context from a local Chroma index.
...
flowchart LR
    ...
    subgraph Private["Private cluster (kind-private)"]
        ...
        Chroma["private Chroma index\n(data/chroma/private)"]
        Orchestrator -->|"query"| Chroma
        Chroma -->|"private chunks"| Orchestrator
    end
...
The private Chroma index is built from `data/private/` (git-tracked synthetic
documents) via `make seed`, or lazily on first query if it doesn't exist yet.
It is persisted under `data/chroma/` (gitignored) and never leaves the
private cluster (see [`specs/v3-spec.md`](specs/v3-spec.md)).
...
- **chromadb (ONNX `all-MiniLM-L6-v2`)** — local vector store for V3's
  private retrieval. Chroma's default embedding function bundles its own
  ONNX model, so no separate model download or GPU is needed; the index is a
  persistent on-disk directory (`data/chroma/private`, gitignored), seeded
  from `data/private/` by `make seed` or lazily on first query.
