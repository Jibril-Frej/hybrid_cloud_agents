"""Unit tests for public.ingest."""

import chromadb

from public.ingest import COLLECTION_NAME, build_index, load_documents
...
class TestBuildIndex:
    ...
    def test_build_index_uses_file_stems_as_ids(self, tmp_path):
        ...
        data_dir = tmp_path / "data"
        data_dir.mkdir()
        (data_dir / "release-notes.md").write_text("Release notes content")
        persist_dir = tmp_path / "index"

        collection = build_index(data_dir, persist_dir)

        # Query to ensure the document is there
        results = collection.get(ids=["release-notes"])
        assert len(results["ids"]) == 1
        assert results["documents"][0] == "Release notes content"

    def test_build_index_is_queryable(self, tmp_path):
        """build_index creates a queryable collection."""
        data_dir = tmp_path / "data"
        data_dir.mkdir()
        (data_dir / "product.md").write_text("Product overview and features")
        (data_dir / "support.md").write_text("Support contact information")
        persist_dir = tmp_path / "index"

        collection = build_index(data_dir, persist_dir)

        results = collection.query(query_texts=["product features"], n_results=1)
        # The product doc should rank higher than the support doc
