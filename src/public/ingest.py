"""Seed the public Chroma vector store from ``data/public/``.

Runs in the public cluster. Builds a persistent Chroma collection with one
entry per Markdown document under ``data/public/``, embedded with Chroma's
default ONNX ``all-MiniLM-L6-v2`` model — the same setup used for the private
index (see ``private.ingest``).
"""
...
def build_index(data_dir: Path, persist_dir: Path) -> chromadb.Collection:
    """Build the public Chroma collection from the documents in ``data_dir``.

    Any existing collection at ``persist_dir`` is replaced.

    Args:
        data_dir: Directory containing the public Markdown documents.
...
if __name__ == "__main__":
    ...
    collection = build_index(PUBLIC_DATA_DIR, PUBLIC_INDEX_DIR)
    print(f"Seeded {collection.count()} document(s) into {PUBLIC_INDEX_DIR}")
