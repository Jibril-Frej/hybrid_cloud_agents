...
**Current version spec: [`specs/v4-spec.md`](../specs/v4-spec.md)**
...
2. Forward — orchestrator sends **only the query** to the public worker over
   mutual TLS (mTLS).
3. Public retrieve — the public worker queries its own local public Chroma
   index for the top-matching public chunks and returns them in its
   response.
4. Private retrieve — in parallel, the orchestrator queries the local
   private Chroma index for the top-matching private chunks. This never
   crosses the boundary.
5. Combine — the public worker's chunks and the private chunks are combined
   into the final answer (templated, not yet LLM-synthesized).
6. Return — the response is returned to the user inside the private
   environment.

This flow gains steps as later versions land (local synthesis in V5) — see
`specs/index.md` for the full sequence. Each `vN-spec.md` documents the
request flow for that version.
