.PHONY: clusters-up clusters-down certs load-certs seed build load-images deploy test test-e2e dev requirements
...
# Regenerate requirements.txt from pyproject.toml. Run this after adding a dependency.
requirements:
